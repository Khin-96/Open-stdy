...
**Turn any lecture, PDF, or video into study notes, flashcards, quizzes, and a study chat with AI proctoring — free, and private by default.**
...
| --- | --- |
| 🍎 **Mac** — Apple Silicon (M1 / M2 / M3 / M4 — most Macs since ~2020) | **[⬇ Download for Mac](https://github.com/Blueturboguy07/Open-Stdy/releases/latest/download/Open-Stdy-mac-arm64.dmg)** |
| 🍎 **Mac** — older Intel models | **[⬇ Download for Intel Mac](https://github.com/Blueturboguy07/Open-Stdy/releases/latest/download/Open-Stdy-mac-x64.dmg)** |
| 🪟 **Windows** — 10 or 11 | **[⬇ Download for Windows](https://github.com/Blueturboguy07/Open-Stdy/releases/latest/download/Open-Stdy-Setup-Windows.exe)** |
...
**After it downloads,** open the file and follow the installer. The very first time you launch Open-Stdy:
...
> **Just want to use Open-Stdy?** The links above are everything you need — enjoy. **Are you a developer?** Don't download the installer; [run it from source](#for-developers) instead so you have the code. You can also browse all versions on the [Releases page](https://github.com/Blueturboguy07/Open-Stdy/releases/latest).

---

Open-Stdy is an open-source, local-first study app with AI proctoring capabilities. Point it at a document, a website, a YouTube link, or an audio file and it generates clean notes (with math), spaced-repetition flashcards, quizzes, and a chat that knows your material. You can run it **fully locally** (no account, no cloud, nothing leaves your machine) or **bring your own** Google Gemini / Groq / Anthropic key for top-tier quality. There is no Open-Stdy subscription, ever.

**New Feature: AI Proctor** - Use Google Gemini Live to monitor your study sessions with real-time video and audio streaming.
...
> [!NOTE]
> **Windows and macOS are both tested and working.** One Windows caveat: automatic setup of the local AI runtime (Ollama) isn't wired up there yet — on Windows, install [Ollama](https://ollama.com/download) once and Open-Stdy will use it, or just use a cloud key. (On macOS it's fully automatic.)
...
If you don't see "Run anyway," instead **right-click the downloaded `Open-Stdy-Setup-Windows.exe` → Properties**, tick **Unblock** near the bottom, click **OK**, then run it.
...
Open-Stdy is a small desktop shell around a local web app. When you open it, the app **starts a tiny local server on your machine**, shows it in a window, keeps it alive, and shuts it down when you quit. That local server is what does the things a plain web page can't — extracting YouTube transcripts with `yt-dlp` and managing the local AI runtime — so **you never install those tools by hand.**

### Three ways to run the AI

1. **Google Gemini** (Cloud) - For audio transcription, generation, and live proctoring
2. **Groq** (Cloud) - Ultra-fast chat completions with Llama models
3. **Local** - Ollama with automatic setup
